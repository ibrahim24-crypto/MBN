...
export const AppLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const filteredNav = navItems.filter(item => {
    // Both SuperAdmin and Administration role can see the Admin Panel
    if (item.href === '/admin') return isSuperAdmin || profile?.role === 'administration';
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="flex flex-col items-center gap-2">
                  ...
                  <Badge variant="outline" className="text-[8px] font-black uppercase tracking-tighter bg-primary/5 text-primary border-primary/20 scale-75">BETA V2</Badge>
                </div>
              </TooltipTrigger>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="md:hidden flex items-center justify-between p-4 border-b dark:border-slate-800 bg-white dark:bg-slate-900 z-50">
          <div className="flex items-center gap-3">
            ...
            <Badge variant="outline" className="text-[8px] font-black bg-primary/5 text-primary border-primary/20">BETA V2</Badge>
          </div>
